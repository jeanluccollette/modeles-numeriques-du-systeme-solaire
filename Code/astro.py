...
Liste des fonctions :
---------------------
planetes(Planetes='Planetes.csv')
lire_info_csv(nom)
liste_id()
...
--------------------
@author Jean-Luc Collette (13/06/2024)
"""
...
def planetes(Planetes='Planetes.csv'):
    chaine = """
Nom,mu,units,id
...
def orbite_kepler(Astre1='Soleil', Astre2='Mercure', Planetes='Planetes.csv',
    ...
    df_plank = pd.DataFrame({'Nom': [Astre1_k, Astre2_k], 'mu': [
                            mu, 0], 'units': ['km^3 s^-2', 'km^3 s^-2']})
    ...
    PV1 = np.array(
        df_Astre1.loc[0, ['X', 'Y', 'Z', 'VX', 'VY', 'VZ']].tolist())
    PV2 = np.array(
        df_Astre2.loc[0, ['X', 'Y', 'Z', 'VX', 'VY', 'VZ']].tolist())
    PV = PV2-PV1
...
def param_orb_comp(Astre1_a='Soleil', Astre2_a='Mercure', Astre1_b='Soleil_simu', Astre2_b='Mercure_simu', Planetes='Planetes.csv'):
    ...
    plt.title('Erreur sur la longitude periastre ' +
              Astre2_a+' (origine '+Astre1_a+')')
...
def equ_dyn_systsol(y, mu):
    ...
    for n in range(N):
        ...
        for k in range(N):
            if k != n:
                Pk = y[6*k:6*k+3]
                yp[6*n+3:6*n+6] = yp[6*n+3:6*n+6]-mu[k] * \
                    (Pn-Pk)/np.sqrt(sum((Pn-Pk)**2))**3
    return yp
...
def trace3D_traj(Astre1, Astre2, XYZ):
    ...
    ax.set_xlabel('X (en M km)'), ax.set_ylabel(
        'Y (en M km)'), ax.set_zlabel('Z (en M km)')
    ax.legend()
...
def simu_systsol_save(Planetes='Planetes.csv', methode='rk8'):
    ...
    for n in range(N):
        ...
        y0 = np.concatenate(
            (y0, liste_df[n].loc[0, ['VX', 'VY', 'VZ']].values))
    y0 = y0.flatten()
    ...
    for n in range(N):
        ...
        df_Astre_simu.to_csv(
            df_planetes.loc[n, 'Nom']+'_simu.csv', index=False)
